#Algorithm taken from BMC Respository by Marcos Duarte
#The algoritm was modified to handle a set of data and not just one array
...
def detect_cusum(x, threshold=2, drift=[0,0,0], ending=False, show=True, ax=None):

    """Cumulative sum algorithm (CUSUM) to detect abrupt changes in data.

    Parameters
    ----------
    x : nD array_like -> Modification to thsi
        data.
    ...
    Returns
    -------
    ta : nD array_like [indi, indf], int
        alarm time (index of when the change was detected).
    tai : nD array_like, int
        index of when the change started.
    taf : nD array_like, int
        index of when the change ended (if `ending` is True).
    amp : nD array_like, float
    ...
    """
    x = np.array(x)
    gp, gn = np.zeros(x.shape), np.zeros(x.shape)
    ...
    for i in range(1,x.shape[0]-1):
        ...
        for j in range(0,2):
            ...
            if gn[i,j] < 0:
                gn[i,j], tan = 0, i
            if gp[i,j] > threshold or gn[i,j] > threshold:  # change detected!
                ta = np.append(ta, i)    # alarm index
                tai = np.append(tai, tap if gp[i,j] > threshold else tan)  # start
                gp[i,j], gn[i,j] = 0, 0      # reset alarm
